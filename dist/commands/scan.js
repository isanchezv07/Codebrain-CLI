...
import { formatIssue, formatGrade, printSummaryTable, buildTree, printTree, } from "../utils/display.js";
import { readFilesRecursive } from "../core/utils.js";
import { aiArchitectureReview, getApiKey } from "../ai/client.js";
import path from "path";
export async function scanCommand(dir) {
    ...
    spinner.succeed(chalk.green(`Scanned ${result.stats.files} files in ${result.duration}ms`));
    // Filter by severity
    ...
    // Sort: critical first
    filtered.sort((a, b) => severityOrder.indexOf(b.severity) - severityOrder.indexOf(a.severity));
    // Show file tree
    ...
    if (apiKey) {
        ...
        if (doAI) {
            ...
            try {
                ...
                const review = await aiArchitectureReview(structure, statsStr, result.issues.filter((i) => i.severity === "critical" || i.severity === "error"), apiKey);
