...
import { safeRead, detectLang, getFileStats } from "../core/utils.js";
...
export async function explainCommand(file) {
    ...
    const allIssues = [...secIssues, ...qualIssues].filter((i) => i.severity === "critical" || i.severity === "error" || i.severity === "warning");
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function complexityLabel(n) {
    if (n <= 5)
        return chalk.green(`${n} (simple)`);
    if (n <= 15)
        return chalk.yellow(`${n} (moderate)`);
    if (n <= 30)
        return chalk.red(`${n} (complex)`);
