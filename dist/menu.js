...
function printHeader() {
    ...
    console.log(chalk.dim("  The ultimate code analysis CLI  ") +
        chalk.dim("v2.0") +
        "  " +
        aiStatus);
    console.log(chalk.dim("  by ") + chalk.hex("#00CCFF").bold("Isanchezv"));
    console.log(chalk.dim("─".repeat(65)));
    if (!apiKey) {
        console.log();
        console.log(chalk.yellow("  🤖 Enable AI mode (Claude) in 2 steps:"));
        console.log();
        console.log(chalk.dim("  1. Get your API key at ") +
            chalk.cyan("https://console.anthropic.com"));
        console.log();
        console.log(chalk.dim("  2. Set it in your terminal:"));
        console.log();
        console.log(chalk.dim("     One session only:   ") +
            chalk.white("export ANTHROPIC_API_KEY=sk-ant-..."));
        console.log(chalk.dim("     Permanent (zsh):    ") +
            chalk.white("echo 'export ANTHROPIC_API_KEY=sk-ant-...' >> ~/.zshrc && source ~/.zshrc"));
        console.log(chalk.dim("     Permanent (bash):   ") +
            chalk.white("echo 'export ANTHROPIC_API_KEY=sk-ant-...' >> ~/.bashrc && source ~/.bashrc"));
        console.log();
        console.log(chalk.dim("  ⚠  Never paste your key inside the code or commit it to git."));
        console.log(chalk.dim("─".repeat(65)));
    }
    console.log();
}
                    ...
                    const { file } = await inquirer.prompt([
                        {
                            ...
                            validate: (v) => v.trim() ? true : "Enter a file path",
                        },
                    ]);
