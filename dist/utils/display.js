...
export function formatIssue(issue, root) {
...
// ─── Grade Display ────────────────────────────────────────────────────────────
export function formatGrade(score, grade) {
    ...
    const gradeColor = grade === "S" || grade === "A"
        ...
        : grade === "B"
            ...
            : grade === "C"
                ? chalk.yellow.bold
                : chalk.red.bold;
    const emoji = score >= 95 ? "🏆" : score >= 85 ? "🔥" : score >= 75 ? "✅" : score >= 60 ? "⚠️" : "💀";
    return [
...
// ─── Summary Table ────────────────────────────────────────────────────────────
export function printSummaryTable(result, root) {
    const bySeverity = {
    ...
    for (const [cat, count] of sorted) {
        ...
        console.log(`  ${icon} ${chalk.bold(cat.padEnd(14))} ${chalk.cyan(count.toString().padStart(3))}  ${chalk.dim(bar)}`);
    }
    ...
    if (s.largestFile)
        console.log(`  Largest file   : ${chalk.dim(s.largestFile)}`);
    if (s.mostIssues)
        console.log(`  Most issues    : ${chalk.dim(s.mostIssues)}`);
...
export function buildTree(files, root) {
    const tree = { name: root.split("/").pop() ?? root, type: "dir", children: [] };
    for (const file of files) {
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            if (!isFile && node.type === "dir")
                cur = node;
        }
    }
    return tree;
}
export function printTree(node, prefix = "", isLast = true) {
