...
export async function bugsCommand(dir: string): Promise<void> {
  ...
  const bugs = result.issues.filter(
    (i) =>
      i.severity === "critical" ||
      i.severity === "error" ||
      i.severity === "warning"
  ...
  spinner.succeed(
    `Static analysis: ${chalk.red(bugs.filter(i => i.severity === "critical").length + " critical")}, ` +
