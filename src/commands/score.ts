...
export async function scoreCommand(dir: string): Promise<void> {
  ...
  for (const cat of categories) {
    ...
    for (const i of catIssues) {
      if (i.severity === "critical") catScore -= 20;
      else if (i.severity === "error") catScore -= 10;
      else if (i.severity === "warning") catScore -= 3;
    }
    catScore = Math.max(0, catScore);

    const bar = buildMiniBar(catScore);
    const color =
      catScore >= 85 ? chalk.green : catScore >= 70 ? chalk.yellow : chalk.red;

    console.log(
      `  ${cat.padEnd(14)} ${bar} ${color(catScore.toString().padStart(3) + "/100")}  ` +
  ...
  if (topIssues.length > 0) {
    ...
    for (const issue of topIssues) {
      ...
      console.log(
        `  ${chalk.red("•")} ${chalk.bold(issue.code)} — ${issue.message}`
      );
