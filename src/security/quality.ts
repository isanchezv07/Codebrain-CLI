...
interface QualityRule {
  code: string;
  pattern?: RegExp;
  check?: (content: string, lines: string[]) => boolean;
  getLine?: (content: string) => number | undefined;
  message: string | ((content: string) => string);
  suggestion: string;
...
const QUALITY_RULES: QualityRule[] = [
  ...
  },
  {
    ...
    check: (content) =>
      content.includes("Promise.all(") && !content.includes(".catch(") && !content.includes("try"),
...
export function scanQuality(file: string, content: string): Issue[] {
  const issues: Issue[] = [];
  ...
  for (const rule of QUALITY_RULES) {
    if (rule.pattern) {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        if (rule.severity === "info" && issues.some(i => i.code === rule.code && i.file === file)) break;
